...
export function MediaUpload({
  ...
  const handleFiles = async (files: FileList | null) => {
    ...
    if (uploadError) {
      setError(uploadError.message || `Unable to upload ${kind}.`)
  ...
  const handleRemove = async () => {
    ...
    if (value?.path) {
      ...
      if (removeError) {
        setError(removeError.message || `Unable to delete ${kind}.`)
        setIsUploading(false)
        return
      }
    }
      ...
      >
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <div className="h-20 w-20 overflow-hidden rounded-md bg-black">
              {previewUrl ? (
                kind === "video" ? (
                  <video
                    src={previewUrl}
                  ...
                  <img src={previewUrl} alt={value?.alt || "Preview"} className="h-full w-full object-cover" />
                )
              ) : (
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-muted-foreground">
                {helperText ??
                  (kind === "video"
                    ? "MP4/WebM recommended, up to 50MB."
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            ...
            >
              {isUploading ? "Uploading..." : `Choose ${kind}`}
            </Button>
            {value?.src ? (
              <Button
                type="button"
